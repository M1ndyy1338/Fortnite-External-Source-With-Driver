...
{
	float bing = driver.read<float>(mesh + 0x330);
	float bong = driver.read<float>(mesh + 0x338);
	const float tick = 0.06f;
	return bong + tick >= bing;
}
void AimAt(DWORD_PTR entity)
{
	uint64_t currentactormesh = driver.read<uint64_t>(entity + 0x310);
...
Uworld = driver.read<DWORD_PTR>(base_address + GWorld);
...
DWORD_PTR PlayerCameraManager = driver.read<DWORD_PTR>(PlayerController + 0x340);
...
uintptr_t Crrneytwep = driver.read<uintptr_t>(LocalPawn + OFFSETS::CurrentWeapon);
...
uint64_t CurrentVehicle = driver.read<uint64_t>(LocalPawn + OFFSETS::CurrentVehicle); //FortPlayerPawn::CurrentVehicle
...
for (uint32_t i = 0; i < Num; i++)
{
	...
	auto CurrentActor = driver.read<uintptr_t>(player + OFFSETS::PawnPrivate);//PawnPrivate
